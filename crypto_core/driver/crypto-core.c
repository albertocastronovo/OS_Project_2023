...
#define REG_KEY_HEX	0x148
#define REG_IV_HEX	0x150
#define REG_IN_HEX	0x158
#define REG_OUT_HEX	0x160
...
static ssize_t ct_show_out_hex(
	struct device *dev, struct device_attribute *attr, char *buf
)
{
	struct crypto_core *ct = dev_get_drvdata(dev);
...
static ssize_t hex_to_uint8(uint8_t *dest, size_t count, const char *src)
{
	char int_buf[3];
	ssize_t i;
	uint8_t value;
	for(i = 0; i < count && *src; i += 1)
	{
		int_buf[0] = *src++;
		int_buf[1] = '\0';
		if(*src)
...
static ssize_t ct_store_key_hex(
	struct device *dev, struct device_attribute *attr, const char *buf, size_t len
)
{
	const char *return_buf = "101";
	...
	uint8_t from_hex[32];
	hex_to_uint8(from_hex, 32, buf);

	for(uint8_t i = 0; i < 8; i += 1)
	{
		key_to_32[i] = char_to_uint32(from_hex+i*4);
	}
	sprintf(temp_buf, "%lu", (unsigned long)key_to_32[0]);
	cc_store(dev, attr, temp_buf, sizeof(temp_buf), REG_KEY_0);
	sprintf(temp_buf, "%lu", (unsigned long)key_to_32[1]);
...
static ssize_t ct_store_iv_hex(
	struct device *dev, struct device_attribute *attr, const char *buf, size_t len
)
{
	const char *return_buf = "202";
	...
	uint8_t from_hex[16];
	hex_to_uint8(from_hex, 16, buf);


	for(uint8_t i = 0; i < 4; i += 1)
	{
		iv_to_32[i] = char_to_uint32(from_hex+i*4);
	}
...
static ssize_t ct_store_in_hex(
	struct device *dev, struct device_attribute *attr, const char *buf, size_t len
)
{
	const char *return_buf = "202";
	...
	for(uint8_t i = 0; i < 4; i += 1)
	{
		in_to_32[i] = char_to_uint32(from_hex+i*4);
	}
...
static DEVICE_ATTR(key_hex,	S_IWUSR,		NULL,		ct_store_key_hex);
...
static DEVICE_ATTR(iv_hex,	S_IWUSR,		NULL,		ct_store_iv_hex);


static DEVICE_ATTR(in_0, 	S_IWUSR, 		NULL,		ct_store_in_0);
static DEVICE_ATTR(in_1, 	S_IWUSR, 		NULL,		ct_store_in_1);
static DEVICE_ATTR(in_2, 	S_IWUSR, 		NULL,		ct_store_in_2);
static DEVICE_ATTR(in_3, 	S_IWUSR, 		NULL,		ct_store_in_3);

static DEVICE_ATTR(in_char,	S_IWUSR,		NULL,		ct_store_in_char);
static DEVICE_ATTR(in_hex,	S_IWUSR,		NULL,		ct_store_in_hex);
...
static DEVICE_ATTR(out_hex,	S_IRUGO,		ct_show_out_hex,	NULL);
